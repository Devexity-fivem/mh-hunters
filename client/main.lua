...
local function SetPedOutfit(ped)
    ...
    local hearTexture = math.random(1, 1)
    local hearItem = math.random(1, 1)
    ...
    if event == "CEventNetworkEntityDamage" then
        if LocalPlayer.state.isLoggedIn then
            ...
            if GetEntityType(victim) == 1 and IsPedHuman(victim) and GetEntityHealth(victim) <= 0 and attacker == PlayerPedId() then
                if (not exports.wasabi_ambulance:isPlayerDead()) and (Config.PedAttackCallHunters == true) then
                    hasNotify = true
...
CreateThread(function()
    while true do
        local sleep = 1000
        if isActive and not exports.wasabi_ambulance:isPlayerDead() then
            sleep = 5
...
AddEventHandler('gameEventTriggered', function(event, data)
    if event ~= "CEventNetworkEntityDamage" or not LocalPlayer.state.isLoggedIn then return end

    local victim   = data[1]
    local attacker = data[2]

    if victim ~= PlayerPedId() then return end
    if not attacker or not DoesEntityExist(attacker) or not isHunterPed(attacker) then return end
    if jailedFromHunters or hunterKillPending then return end

    hunterKillPending = true

    CreateThread(function()
        local minutes = Config.HunterJailTime or 20
        local reason  = "caught by hunters"

        -- 1) wait for wasabi_ambulance to actually flag us as dead (up to 5s)
        local waited, dead = 0, false
        if GetResourceState('wasabi_ambulance') == 'started' then
            ...
            -- fallback if wasabi not present
            dead = (GetEntityHealth(PlayerPedId()) <= 0)
        end

        if not dead then
            hunterKillPending = false
            return
        end

        -- 2) don't wait for revive here; let the server revive, then jail
        jailedFromHunters = true
        TriggerServerEvent('mh-hunters:server:reviveAndJailFromHunters', minutes, reason)
